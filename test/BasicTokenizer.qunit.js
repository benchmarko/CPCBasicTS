...
qunit_1.QUnit.module("BasicTokenizer:decode: Tests", function () {
    var mAllTests = {
        PRG: {
    ...
    function runTestsFor(assert, oTests, aResults) {
        ...
        for (var sKey in oTests) {
            if (oTests.hasOwnProperty(sKey)) {
                var sInput = Utils_1.Utils.atob(sKey), // decode base64
                sResult = oBasicTokenizer.decode(sInput), sExpected = oTests[sKey], sFirstLine = sExpected.substr(0, sExpected.indexOf("\n"));
                if (aResults) {
